			...
			<div class="filters">
				...
				<div>
						...
						<div class="toggle-section">
							<div class="facets">
								<label v-for="(level, index) in getArchivalFacets.archival_level" v-bind:key="index" class="facet">
									<input type="checkbox" name="level" v-bind:aria-label="level.key" :value="level.key" v-on:click="filter('archival_level', level.key)" :checked="checkedOption('archival_level', level.key)"/> {{level.key}} <span class="count">({{level.doc_count}})</span>
								</label>
							</div>
						</div>
					</fieldset>
						...
						<div class="toggle-section">
							...
							<div class="facets">
								<label v-for="(type, index) in sortedData(getArchivalFacets.record_types, searchRecordTypes, 'count')" v-bind:key="index" class="facet" v-show="index < 5 || recordTypeCheckbox">
									<input type="checkbox" name="type" v-bind:aria-label="type.key" :value="type.key" v-on:click="filter('record_types', type.key)" :checked="checkedOption('record_types', type.key)"/> {{type.key}} <span class="count">({{type.doc_count}})</span>
								</label>
							</div>
					...
					<fieldset v-if="getArchivalFacets.creators && getArchivalFacets.creators.length > 0">
						...
						<div class="toggle-section">
							<input v-if="getArchivalFacets.creators.length > 5" type="text" aria-label="Search writer" placeholder="Search writer" onfocus="this.placeholder=''" v-on:click="writersCheckbox = true" v-model="searchWriters" onblur="this.placeholder='Search writer'" name="writer_search"/>
							<div class="facets">
							<label v-for="(writer, index) in sortedData(getArchivalFacets.creators, searchWriters, 'alphabetical')" v-bind:key="index" class="facet" v-show="index < 5 || writersCheckbox">
								<input type="checkbox" name="writer" v-bind:aria-label="writer.key" :value="writer.key" v-on:click="filter('creators', writer.key)" :checked="checkedOption('creators', writer.key)"/> {{writer.key}} <span class="count">({{writer.doc_count}})</span>
							</label>
							</div>
					...
					<fieldset v-if="getArchivalFacets.persons_as_relations && getArchivalFacets.persons_as_relations.length > 0">
						...
						<div class="toggle-section">
							...
							<label v-for="(addressee, index) in sortedData(getArchivalFacets.persons_as_relations, searchAddressees, 'alphabetical')" v-bind:key="index" class="facet" v-show="index < 5 || addresseesCheckbox">
								<input type="checkbox" name="addressee" :value="addressee.key" v-bind:aria-label="addressee.key" v-on:click="filter('person_as_relations', addressee.key)" :checked="checkedOption('person_as_relations', addressee.key)"/> {{addressee.key}} <span class="count">({{addressee.doc_count}})</span>
							</label>
							</div>
					...
					<fieldset v-if="getArchivalFacets.languages && getArchivalFacets.languages.length > 0">
						...
						<div class="toggle-section">
							...
							<label v-for="(language, index) in sortedData(getArchivalFacets.languages, searchLanguages, 'count')" v-bind:key="index" class="facet" v-show="index < 5 || languagesCheckbox">
								<input type="checkbox" name="language" :value="language.key" v-bind:aria-label="language.key" v-on:click="filter('language', language.key)" :checked="checkedOption('language', language.key)"/> {{language.key}} <span class="count">({{language.doc_count}})</span>
							</label>
							</div>
